  ...
  public class WorldFile
  {
    ...
    private static void DoRollingBackups(string backupWorldWritePath)
    {
      ...
      for (int index = num2 - 1; index > 0; --index)
      {
        ...
        if (FileUtilities.Exists(str, false))
          FileUtilities.Move(str, destination, false, true);
      }
    }
    ...
    private static void ConvertOldTileEntities()
    {
      ...
      foreach (Point point in pointList1)
      {
        if (WorldGen.InWorld(point.X, point.Y, 5))
        {
          ...
          int index = TEDisplayDoll.Place(point.X, point.Y);
          if (index != -1)
            (TileEntity.ByID[index] as TEDisplayDoll).SetInventoryFromMannequin(frameX1, frameX2, frameX3);
        }
      }
      foreach (Point point in pointList2)
      {
        if (WorldGen.InWorld(point.X, point.Y, 5))
        {
          ...
          int frameX1 = (int) Main.tile[point.X, point.Y + 1].frameX;
          int frameX2 = (int) Main.tile[point.X + 1, point.Y + 1].frameX;
          bool flag2 = frameX1 >= 5000;
          int netid = frameX1 % 5000 - 100;
          int prefix = frameX2 - (frameX2 >= 25000 ? 25000 : 10000);
          for (int index1 = 0; index1 < 3; ++index1)
          {
            for (int index2 = 0; index2 < 3; ++index2)
            {
              ...
              tile.frameX = (short) ((flag1 ? 54 : 0) + index1 * 18);
    ...
    private static int SaveWorldTiles(BinaryWriter writer)
    {
      ...
      for (int index1 = 0; index1 < Main.maxTilesX; ++index1)
      {
        ...
        for (int index2 = 0; index2 < Main.maxTilesY; index2 = num2 + 1)
        {
          ...
          short num9 = 0;
          int index8 = index2 + 1;
          for (int index4 = Main.maxTilesY - index2 - 1; index4 > 0 && tile.isTheSameAs(Main.tile[index1, index8]); ++index8)
          {
            ++num9;
            --index4;
          }
      ...
      for (int index1 = 0; index1 < Main.maxTilesX; ++index1)
      {
        ...
        for (int index2 = 0; index2 < Main.maxTilesY; ++index2)
        {
          ...
          if (num2 > (byte) 0)
          {
            ...
            {
              byte num5 = reader.ReadByte();
              from.wall = (ushort) ((uint) num5 << 8 | (uint) from.wall);
              if (from.wall >= (ushort) 316)
                from.wall = (ushort) 0;
            }
          }
    ...
    private static void LoadNPCs(BinaryReader reader)
    {
      ...
      for (bool flag = reader.ReadBoolean(); flag; flag = reader.ReadBoolean())
      {
        ...
        if (WorldFile._versionNumber >= 190)
          npc.SetDefaults(reader.ReadInt32(), new NPCSpawnParams());
        else
          npc.SetDefaults(NPCID.FromLegacyName(reader.ReadString()), new NPCSpawnParams());
    ...
    private static bool ValidateWorld(BinaryReader fileIO)
    {
      ...
      try
      {
        ...
        int num17 = fileIO.ReadBoolean() ? 1 : 0;
        string str2 = fileIO.ReadString();
        int num18 = fileIO.ReadInt32();
        bool flag = false;
        if (num17 != 0 && (str2 == str1 || num18 == num3))
          flag = true;
        return flag;
      }
      catch (Exception ex)
    ...
    private static int LoadWorld_Version1_Old_BeforeRelease88(BinaryReader fileIO)
    {
            ...
            if (index2 < num5)
            {
              ...
              if (num1 > 0)
              {
                ...
                else
                {
                  ...
                  Main.chest[index1].item[index2].SetDefaults((int) num2);
                }
      ...
      for (int index1 = 0; index1 < 1000; ++index1)
      {
        if (fileIO.ReadBoolean())
        {
          ...
          if (Main.tile[index2, index3].active() && (Main.tile[index2, index3].type == (ushort) 55 || Main.tile[index2, index3].type == (ushort) 85))
          {
      ...
      while (flag)
      {
        if (versionNumber >= 190)
          Main.npc[index].SetDefaults(fileIO.ReadInt32(), new NPCSpawnParams());
        else
          Main.npc[index].SetDefaults(NPCID.FromLegacyName(fileIO.ReadString()), new NPCSpawnParams());
        if (versionNumber >= 83)
      ...
      if (versionNumber < 7)
        return 0;
      int num7 = fileIO.ReadBoolean() ? 1 : 0;
      string str1 = fileIO.ReadString();
      int num8 = fileIO.ReadInt32();
      return num7 != 0 && (str1 == Main.worldName || num8 == Main.worldID) ? 0 : 2;
    }
  }
}
