  ...
  public static class PlayerDataInitializer
  {
    ...
    private static void LoadVariant(int ID, int[] pieceIDs)
    {
      for (int index = 0; index < pieceIDs.Length; ++index)
        TextureAssets.Players[ID, pieceIDs[index]] = (Asset<Texture2D>) Main.Assets.Request<Texture2D>("Images/Player_" + (object) ID + "_" + (object) pieceIDs[index], (AssetRequestMode) 2);
    }
