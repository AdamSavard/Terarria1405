...
namespace Terraria.Social.WeGame
{
  public class NetClientSocialModule : NetSocialModule
  {
    ...
    private string DumpMataDataString(List<RailKeyValueResult> list)
    {
      ...
      using (List<RailKeyValueResult>.Enumerator enumerator = list.GetEnumerator())
      {
        while (enumerator.MoveNext())
        {
          RailKeyValueResult current = enumerator.Current;
          stringBuilder.Append("key: " + (string) current.key + " value: " + (string) current.value);
        }
      }
    ...
    private string GetFriendNickname(RailID rail_id)
    {
      ...
      {
        using (List<PlayerPersonalInfo>.Enumerator enumerator = this._player_info_list.GetEnumerator())
        {
          while (enumerator.MoveNext())
          {
    ...
    {
      this._player_info_list = (List<PlayerPersonalInfo>) data.user_info_list;
    }
