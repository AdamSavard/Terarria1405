    ...
    public void ReadTick()
    {
      ...
      lock (this._pendingReadBuffers)
      {
        ...
        while (this.IsPacketAvailable(out size))
        {
          ...
          RailID index = new RailID();
          bool flag;
          lock (this.RailLock)
            flag = rail_api.RailFactory().RailNetworkHelper().ReadData(this.GetLocalPeer(), index, data, size) == 0;
          if (flag)
          {
            if (this._readEvent == null || this._readEvent(data, (int) size, index))
            {
              if (!this._pendingReadBuffers.ContainsKey(index))
                this._pendingReadBuffers[index] = new Queue<WeGameP2PReader.ReadResult>();
              this._pendingReadBuffers[index].Enqueue(new WeGameP2PReader.ReadResult(data, size));
            }
            else
