...
namespace Terraria.Social.WeGame
{
  public class WeGameP2PWriter
  {
    ...
    public void SendAll()
    {
      ...
      using (Dictionary<RailID, Queue<WeGameP2PWriter.WriteInformation>>.Enumerator enumerator = this._pendingSendDataSwap.GetEnumerator())
      {
        while (enumerator.MoveNext())
        {
          KeyValuePair<RailID, Queue<WeGameP2PWriter.WriteInformation>> current = enumerator.Current;
          Queue<WeGameP2PWriter.WriteInformation> writeInformationQueue = current.Value;
          while (writeInformationQueue.Count > 0)
          {
            WeGameP2PWriter.WriteInformation writeInformation = writeInformationQueue.Dequeue();
            int num = rail_api.RailFactory().RailNetworkHelper().SendData(this.GetLocalPeer(), current.Key, writeInformation.Data, (uint) writeInformation.Size) == 0 ? 1 : 0;
