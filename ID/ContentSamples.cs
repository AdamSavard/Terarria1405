...
namespace Terraria.ID
{
  public static class ContentSamples
  {
    ...
    public static void Initialize()
    {
      ...
      for (int Type = -65; Type < 663; ++Type)
      {
        NPC npc = new NPC();
        npc.SetDefaults(Type, new NPCSpawnParams());
        ContentSamples.NpcsByNetId[Type] = npc;
        string name = NPCID.Search.GetName(npc.netID);
        ContentSamples.NpcPersistentIdsByNetIds[Type] = name;
        ContentSamples.NpcBestiaryCreditIdsByNpcNetIds[Type] = name;
        ContentSamples.NpcNetIdsByPersistentIds[name] = Type;
      }
      ContentSamples.ModifyNPCIds();
      ContentSamples.ProjectilesByType.Clear();
      for (int Type = 0; Type < 950; ++Type)
      {
        Projectile projectile = new Projectile();
        projectile.SetDefaults(Type);
        ContentSamples.ProjectilesByType[Type] = projectile;
      }
      ...
      for (int Type = 0; Type < 5045; ++Type)
      {
        ...
        ContentSamples.ItemsByType[Type] = obj;
        string name = ItemID.Search.GetName(obj.netID);
        ContentSamples.ItemPersistentIdsByNetIds[Type] = name;
        ContentSamples.ItemNetIdsByPersistentIds[name] = Type;
      }
      foreach (int Type in ItemID.Sets.ItemsThatAreProcessedAfterNormalContentSample)
      {
        Item obj = new Item();
        obj.SetDefaults(Type);
        ContentSamples.ItemsByType[Type] = obj;
    ...
    private static void ModifyNPCIds()
    {
      ...
      short num = 499;
      for (int index = 498; index <= 506; ++index)
        creditIdsByNpcNetIds[index] = creditIdsByNpcNetIds[(int) num];
    ...
    private static void FillNpcRarities()
    {
      ...
      {
        NPC npc = new NPC();
        npc.SetDefaults(Type, spawnparams);
        ContentSamples.NpcBestiaryRarityStars[Type] = ContentSamples.GetNPCBestiaryRarityStarsCount(npc);
      }
