  ...
  public class Gore
  {
      ...
      switch (GoreID.Sets.SpecialAI[this.type])
      {
        ...
        default:
          ...
          else if (this.type == 1208)
          {
            ...
            int index1 = (int) vector2.X / 16;
            int index2 = (int) vector2.Y / 16;
            bool flag2 = Main.tile[index1, index2] != null && Main.tile[index1, index2].liquid > (byte) 0;
            this.scale -= 0.0005f;
            if ((double) this.scale < 0.1)
            {
              this.scale = 0.1f;
            ...
            if (flag2)
            {
              ...
              int index4 = (int) ((double) vector2.Y / 16.0);
              double num = (double) this.position.Y / 16.0;
              int index5 = (int) (((double) this.position.Y + (double) this.Height) / 16.0);
              if (Main.tile[index3, index4] == null)
                Main.tile[index3, index4] = new Tile();
              if (Main.tile[index3, index5] == null)
                Main.tile[index3, index5] = new Tile();
              if ((double) this.velocity.Y > 0.0)
                this.velocity.Y *= 0.5f;
          ...
          else if (GoreID.Sets.SpecialAI[this.type] != 1 && GoreID.Sets.SpecialAI[this.type] != 2)
          {
            ...
            else if (this.type == 1218)
            {
              ...
              if (this.alpha < 100 && (double) this.velocity.Length() > 0.0 && Main.rand.Next(5) == 0)
              {
                ...
                int index = Dust.NewDust(this.position + new Vector2(6f, 4f), 4, 4, Type, 0.0f, 0.0f, 0, new Color(), 1f);
          ...
          if (GoreID.Sets.SpecialAI[this.type] == 2)
          {
            ...
            else
            {
              ...
              if (TextureAssets.Gore[this.type].IsLoaded && (double) num2 != -1.0)
              {
                ...
                float num4 = (float) TextureAssets.Gore[this.type].Width() * this.scale;
                float num5 = (float) TextureAssets.Gore[this.type].Height() * this.scale;
                int Type = 31;
          ...
          if (this.type >= 411 && this.type <= 430)
          {
            ...
            else
            {
              ...
              if (TextureAssets.Gore[this.type].IsLoaded)
              {
                ...
                float x = this.position.X;
                float y = this.position.Y;
                float num2 = (float) TextureAssets.Gore[this.type].Width() * this.scale;
                float num3 = (float) TextureAssets.Gore[this.type].Height() * this.scale;
                int Type = 176;
          ...
          else if (GoreID.Sets.SpecialAI[this.type] != 3 && GoreID.Sets.SpecialAI[this.type] != 1)
          {
            if (this.type >= 706 && this.type <= 717 || (this.type == 943 || this.type == 1147) || this.type >= 1160 && this.type <= 1162)
            {
              ...
              {
                float num1 = 0.6f;
                float num2 = this.frame != (byte) 0 ? (this.frame != (byte) 1 ? (this.frame != (byte) 2 ? (this.frame != (byte) 3 ? (this.frame != (byte) 4 ? (this.frame != (byte) 5 ? (this.frame != (byte) 6 ? (this.frame > (byte) 9 ? (this.frame != (byte) 10 ? (this.frame != (byte) 11 ? (this.frame != (byte) 12 ? (this.frame != (byte) 13 ? (this.frame != (byte) 14 ? 0.0f : num1 * 0.1f) : num1 * 0.2f) : num1 * 0.3f) : num1 * 0.4f) : num1 * 0.5f) : num1 * 0.5f) : num1 * 0.2f) : num1 * 0.4f) : num1 * 0.5f) : num1 * 0.4f) : num1 * 0.3f) : num1 * 0.2f) : num1 * 0.1f;
              ...
              if (this.velocity != velocity)
              {
                if (this.frame < (byte) 10)
                {
                  ...
                  if (this.type != 716 && this.type != 717 && this.type != 943 && (this.type < 1160 || this.type > 1162))
                    SoundEngine.PlaySound(39, (int) this.position.X + 8, (int) this.position.Y + 8, Main.rand.Next(2), 1f, 0.0f);
                }
              }
              else if (Collision.WetCollision(this.position + this.velocity, 16, 14))
              {
                if (this.frame < (byte) 10)
                {
                  ...
                  this.frameCounter = (byte) 0;
                  if (this.type != 716 && this.type != 717 && this.type != 943 && (this.type < 1160 || this.type > 1162))
                    SoundEngine.PlaySound(39, (int) this.position.X + 8, (int) this.position.Y + 8, 2, 1f, 0.0f);
    ...
    private void Gore_UpdateLeaf()
    {
      ...
      if ((double) this.velocity.Y < 0.0)
      {
        ...
        else
        {
          ...
          else
          {
            ...
            if (flag)
            {
              ...
              else if (tile.lava())
              {
                ...
                for (int index = 0; index < 5; ++index)
                  Dust.NewDust(this.position, num1, num1, 31, 0.0f, -0.2f, 0, new Color(), 1f);
              }
              else
            ...
            if (!WorldGen.SolidTile(tileCoordinates.X, tileCoordinates.Y + 1, false) && !flag)
            {
              this.velocity.Y = 0.1f;
              this.timeLeft = 0;
              this.alpha += 20;
            }
            vector2_2 = Collision.TileCollision(Position, vector2_2, num1, num1, false, false, 1);
            if (flag)
      ...
      else
      {
        ...
        Vector2 vector2_2 = new Vector2(Vector2.UnitY.RotatedBy((double) this.velocity.Y, new Vector2()).X * 1f, Math.Abs(Vector2.UnitY.RotatedBy((double) this.velocity.Y, new Vector2()).Y) * 1f);
        int num1 = 4;
        if ((double) this.position.Y < Main.worldSurface * 16.0)
          vector2_2.X += Main.WindForVisuals * 4f;
        Vector2 vector2_3 = vector2_2;
        vector2_2 = Collision.TileCollision(Position, vector2_2, num1, num1, false, false, 1);
        Vector4 vector4 = Collision.SlopeCollision(Position, vector2_2, num1, num1, 1f, false);
        this.position.X = vector4.X;
        this.position.Y = vector4.Y;
        vector2_2.X = vector4.Z;
        vector2_2.Y = vector4.W;
        this.position += vector2_1;
        if (vector2_2 != vector2_3)
          this.velocity.Y = -1f;
        ...
        else
        {
          ...
          else
          {
            ...
            Rectangle rectangle1 = new Rectangle(tileCoordinates.X * 16, tileCoordinates.Y * 16 + (int) tile.liquid / 16, 16, 16 - (int) tile.liquid / 16);
            Rectangle rectangle2 = new Rectangle((int) Position.X, (int) Position.Y + num2, num1, num1);
            if (tile != null && tile.liquid > (byte) 0 && rectangle1.Intersects(rectangle2))
              this.velocity.Y = -1f;
