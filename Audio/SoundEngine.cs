...
namespace Terraria.Audio
{
  public static class SoundEngine
  {
    ...
    {
      return !SoundEngine.IsAudioSupported ? (ActiveSound) null : SoundEngine._player.GetActiveSound(id);
    }

    public static SlotId PlayTrackedSound(SoundStyle style, Vector2 position)
    {
      return !SoundEngine.IsAudioSupported ? (SlotId) SlotId.Invalid : SoundEngine._player.Play(style, position);
    }

    public static SlotId PlayTrackedSound(SoundStyle style)
    {
      return !SoundEngine.IsAudioSupported ? (SlotId) SlotId.Invalid : SoundEngine._player.Play(style);
    }
    ...
    {
      return !SoundEngine.IsAudioSupported ? (SoundEffect) null : SoundEngine._legacyPlayer.GetTrackableSoundByStyleId(id);
    }
    ...
    {
      return !SoundEngine.IsAudioSupported ? (ActiveSound) null : SoundEngine._player.FindActiveSound(style);
    }
