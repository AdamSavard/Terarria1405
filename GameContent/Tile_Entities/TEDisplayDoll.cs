...
namespace Terraria.GameContent.Tile_Entities
{
  public class TEDisplayDoll : TileEntity
  {
    ...
    {
      return (TileEntity) new TEDisplayDoll();
    }
      ...
      int inventoryContextTarget)
    {
      ...
      for (int index1 = 0; index1 < slotsToShowLine; ++index1)
      {
        for (int index2 = 0; index2 < 2; ++index2)
        {
          ...
          ItemSlot.Draw(spriteBatch, inv, context, index1 + slotsArrayOffset, new Vector2((float) num1, (float) num2), new Color());
        }
      }
    }
    ...
    private bool TryFitting(Item[] inv, int context = 0, int slot = 0, bool justCheck = false)
    {
      ...
      if (Main.netMode == 1)
        NetMessage.SendData(121, -1, -1, (NetworkText) null, Main.myPlayer, (float) this.ID, (float) index1, 0.0f, 0, 0, 0);
      return true;
    }
