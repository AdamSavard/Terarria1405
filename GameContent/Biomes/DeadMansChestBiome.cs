...
namespace Terraria.GameContent.Biomes
{
  public class DeadMansChestBiome : MicroBiome
  {
    ...
    private void FindExplosiveTrapSpots(Point position)
    {
      ...
      for (int x4 = num3; x4 < num3 + num2; ++x4)
      {
        if (this.IsGoodSpotsForExplosive(x4, y))
          intList.Add(x4);
      }
      int x6 = -1;
      if (intList.Count > 0)
        x6 = intList[GenBase._random.Next(intList.Count)];
      if (x6 != -1)
        this._explosivePlacementAttempt.Add(new DeadMansChestBiome.ExplosivePlacementAttempt(new Point(x6, y)));
      if (x3 != -1)
      ...
      int bestType)
    {
      for (int x = position.X; x < position.X + 2; ++x)
      {
        for (int j = position.Y - requiredHeight; j <= position.Y + 2; ++j)
        {
          ...
          else if (j <= position.Y)
          {
            ...
            tile.actuator(true);
            tile.wire(true);
            WorldGen.TileFrame(x, j, true, false);
          }
          else
            tile.ClearTile();
        }
      ...
      WorldGen.PlaceTile(i, j1, 138, false, false, -1, 0);
