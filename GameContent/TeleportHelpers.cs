...
namespace Terraria.GameContent
{
  public class TeleportHelpers
  {
    ...
    private static bool IsInSolidTilesExtended(
      ...
      int gravDir)
    {
      if (Collision.LavaCollision(testPosition, width, height) || (double) Collision.HurtTiles(testPosition, playerVelocity, width, height, false).Y > 0.0 || Collision.SolidCollision(testPosition, width, height))
        return true;
      Vector2 Velocity1 = Vector2.UnitX * 16f;
      if (Collision.TileCollision(testPosition - Velocity1, Velocity1, width, height, false, false, gravDir) != Velocity1)
        return true;
      Vector2 Velocity2 = -Vector2.UnitX * 16f;
      if (Collision.TileCollision(testPosition - Velocity2, Velocity2, width, height, false, false, gravDir) != Velocity2)
        return true;
      Vector2 Velocity3 = Vector2.UnitY * 16f;
      if (Collision.TileCollision(testPosition - Velocity3, Velocity3, width, height, false, false, gravDir) != Velocity3)
        return true;
      Vector2 Velocity4 = -Vector2.UnitY * 16f;
      return Collision.TileCollision(testPosition - Velocity4, Velocity4, width, height, false, false, gravDir) != Velocity4;
    }
  }
}
