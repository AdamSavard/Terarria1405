...
namespace Terraria.GameContent
{
  public abstract class AnOutlinedDrawRenderTargetContent : ARenderTargetContentByRequest
  {
    ...
    protected override void HandleUseReqest(GraphicsDevice device, SpriteBatch spriteBatch)
    {
      ...
      int num3 = num1;
      for (int index1 = -num3; index1 <= num3; index1 += num1)
      {
        for (int index2 = -num3; index2 <= num3; index2 += num1)
        {
          if (Math.Abs(index1) + Math.Abs(index2) == num3)
            spriteBatch.Draw((Texture2D) this._helperTarget, new Vector2((float) index1, (float) index2), this._borderColor);
        }
      }
