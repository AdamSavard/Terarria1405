...
namespace Terraria.GameContent.UI.Elements
{
  public class UIBestiaryEntryGrid : UIElement
  {
    ...
    public void FillBestiarySpaceWithEntries()
    {
      ...
      int id = 0;
      float num2 = 0.5f / (float) maxEntriesWidth;
      float num3 = 0.5f / (float) maxEntriesHeight;
      for (int index1 = 0; index1 < maxEntriesHeight; ++index1)
      {
        for (int index2 = 0; index2 < maxEntriesWidth && id < bestiaryEntryList.Count; ++index2)
        {
          UIElement element = (UIElement) new UIBestiaryEntryButton(bestiaryEntryList[id], false);
          ++id;
          element.OnClick += this._clickOnEntryEvent;
          element.VAlign = element.HAlign = 0.5f;
          element.Left.Set(0.0f, (float) ((double) index2 / (double) maxEntriesWidth - 0.5) + num2);
          element.Top.Set(0.0f, (float) ((double) index1 / (double) maxEntriesHeight - 0.5) + num3);
          element.SetSnapPoint("Entries", id, new Vector2?(new Vector2(0.2f, 0.7f)), new Vector2?());
          this.Append(element);
        }
      }
    }
    ...
    {
      element.OnClick += (UIElement.MouseEvent) ((e, v) => this.OffsetLibraryByPages(howManyPages));
    }
