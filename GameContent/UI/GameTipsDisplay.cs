  ...
  public class GameTipsDisplay
  {
    ...
    public void Draw()
    {
      ...
      foreach (GameTipsDisplay.GameTip currentTip in this._currentTips)
      {
        if ((double) currentTip.ScreenAnchorX >= -0.5 && (double) currentTip.ScreenAnchorX <= 1.5)
        {
          ...
          ChatManager.DrawColorCodedStringWithShadow(spriteBatch, font, text, position, Color.White, 0.0f, Vector2.Zero, new Vector2(num2, num2), -1f, 2f);
        }
      }
    }
    ...
    private class GameTip
    {
      ...
      public string Text
      {
        get
        {
          return this._textKey == null ? "What?!" : this._formattedText;
        }
      }
