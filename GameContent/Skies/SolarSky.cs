...
namespace Terraria.GameContent.Skies
{
  public class SolarSky : CustomSky
  {
    ...
    {
      this._planetTexture = (Asset<Texture2D>) Main.Assets.Request<Texture2D>("Images/Misc/SolarSky/Planet", (AssetRequestMode) 1);
      this._bgTexture = (Asset<Texture2D>) Main.Assets.Request<Texture2D>("Images/Misc/SolarSky/Background", (AssetRequestMode) 1);
      this._meteorTexture = (Asset<Texture2D>) Main.Assets.Request<Texture2D>("Images/Misc/SolarSky/Meteor", (AssetRequestMode) 1);
    }
    ...
    public override void Draw(SpriteBatch spriteBatch, float minDepth, float maxDepth)
    {
      ...
      for (int index = num1; index < num2; ++index)
      {
        ...
        if (rectangle.Contains((int) position.X, (int) position.Y))
          spriteBatch.Draw(this._meteorTexture.get_Value(), position, new Rectangle?(new Rectangle(0, num4 * (this._meteorTexture.Height() / 4), this._meteorTexture.Width(), this._meteorTexture.Height() / 4)), Color.White * num3 * this._fadeOpacity, 0.0f, Vector2.Zero, vector2_1.X * 5f * this._meteors[index].Scale, SpriteEffects.None, 0.0f);
      }
    }
