...
namespace Terraria.GameContent.Skies
{
  public class StardustSky : CustomSky
  {
    ...
    {
      this._planetTexture = (Asset<Texture2D>) Main.Assets.Request<Texture2D>("Images/Misc/StarDustSky/Planet", (AssetRequestMode) 1);
      this._bgTexture = (Asset<Texture2D>) Main.Assets.Request<Texture2D>("Images/Misc/StarDustSky/Background", (AssetRequestMode) 1);
      this._starTextures = new Asset<Texture2D>[2];
      for (int index = 0; index < this._starTextures.Length; ++index)
        this._starTextures[index] = (Asset<Texture2D>) Main.Assets.Request<Texture2D>("Images/Misc/StarDustSky/Star " + (object) index, (AssetRequestMode) 1);
    }
    ...
    public override void Draw(SpriteBatch spriteBatch, float minDepth, float maxDepth)
    {
      ...
      for (int index = num1; index < num2; ++index)
      {
        ...
        if (rectangle.Contains((int) position.X, (int) position.Y))
        {
          ...
          Texture2D texture = this._starTextures[this._stars[index].TextureIndex].get_Value();
          spriteBatch.Draw(texture, position, new Rectangle?(), Color.White * num3 * num6 * 0.8f * (1f - num5) * this._fadeOpacity, 0.0f, new Vector2((float) (texture.Width >> 1), (float) (texture.Height >> 1)), (float) (((double) vector2_1.X * 0.5 + 0.5) * ((double) num6 * 0.300000011920929 + 0.699999988079071)), SpriteEffects.None, 0.0f);
        }
      }
    }
