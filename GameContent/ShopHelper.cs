...
namespace Terraria.GameContent
{
  public class ShopHelper
  {
        ...
        else
        {
          ...
          if (npcsWithinHouse > 2)
          {
            ...
            if (npcsWithinHouse > 4)
              this.AddHappinessReportText("HateCrowded", (object) null);
            else
              this.AddHappinessReportText("DislikeCrowded", (object) null);
          }
          if (npcsWithinHouse < 2 && npcsWithinVillage < 4)
          {
            this.AddHappinessReportText("LoveSpace", (object) null);
          ...
          if (this._currentHappiness == "")
            this.AddHappinessReportText("Content", (object) null);
    ...
    {
      if (npc.homeless)
        this.AddHappinessReportText("NoHome", (object) null);
      return npc.homeless;
    }
    ...
    private bool IsFarFromHome(NPC npc)
    {
      if ((double) Vector2.Distance(new Vector2((float) npc.homeTileX, (float) npc.homeTileY), new Vector2(npc.Center.X / 16f, npc.Center.Y / 16f)) <= 120.0)
        return false;
      this.AddHappinessReportText("FarFromHome", (object) null);
      return true;
    }
