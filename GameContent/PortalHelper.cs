        ...
        if (projectile.active && projectile.type == 602)
        {
          ...
          if (Collision.CheckLinevLine(start1, end1, start2, end2).Length != 0)
          {
            if (projectile.owner != Main.myPlayer && Main.netMode != 2)
              NetMessage.SendData(95, -1, -1, (NetworkText) null, projectile.owner, (float) (int) projectile.ai[1], 0.0f, 0.0f, 0, 0, 0);
            projectile.Kill();
          }
        }
      }
    ...
    public static bool SupportedTilesAreFine(Vector2 portalCenter, float portalAngle)
    {
      ...
      switch (num1)
      {
        ...
        case 4:
          ...
          break;
        default:
          Main.NewText("Broken portal! (over4s = " + (object) num1 + " , " + (object) portalAngle + ")", byte.MaxValue, byte.MaxValue, byte.MaxValue);
          return false;
      }
      if (num2 != 0 && num3 != 0)
      {
