...
namespace Terraria.GameContent
{
  public class ChromaHotkeyPainter
  {
    ...
    public bool PotionAlert
    {
      get
      {
        return (uint) this._quickHealAlert > 0U;
      }
    }
    ...
    private class ReactiveRGBKey
    {
      ...
      public void Bind()
      {
        this._rgbKey = Main.Chroma.BindKey(this.XNAKey);
      }
    ...
    private class PaintKey
    {
      ...
      public void Unbind()
      {
        using (List<RgbKey>.Enumerator enumerator = this._rgbKeys.GetEnumerator())
        {
          while (enumerator.MoveNext())
          {
            RgbKey current = enumerator.Current;
            Main.Chroma.UnbindKey((Keys) current.Key);
          }
        }
      }
      ...
      public void SetSolid(Color color)
      {
        ...
        {
          while (enumerator.MoveNext())
            enumerator.Current.SetSolid(color);
        }
      }
      ...
      public void SetAlert(Color colorBase, Color colorFlash, float time, float flashesPerSecond)
      {
        ...
        {
          while (enumerator.MoveNext())
            enumerator.Current.SetFlashing(colorBase, colorFlash, time, flashesPerSecond);
        }
      }

      public List<Keys> GetXNAKeysInUse()
      {
        return new List<Keys>((IEnumerable<Keys>) this._xnaKeys);
      }
    }
