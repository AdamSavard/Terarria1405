...
namespace Terraria.GameContent.Bestiary
{
  public class NPCStatsReportInfoElement : IBestiaryInfoElement
  {
    ...
    public NPCStatsReportInfoElement(int npcNetId, int gameMode)
    {
      ...
      if (!Main.RegisterdGameModes.TryGetValue(this.GameMode, out gameModeData))
        return;
      NPC npc = new NPC();
      npc.SetDefaults(this.NpcId, new NPCSpawnParams()
      {
        gameModeData = gameModeData,
    ...
    private void ShowStats_Attack(UIElement element)
    {
      if (!element.IsMouseHovering)
        return;
      Main.instance.MouseText(Language.GetTextValue("BestiaryInfo.Attack"), 0, (byte) 0, -1, -1, -1, -1, 0);
    }
    ...
    private void ShowStats_Defense(UIElement element)
    {
      if (!element.IsMouseHovering)
        return;
      Main.instance.MouseText(Language.GetTextValue("BestiaryInfo.Defense"), 0, (byte) 0, -1, -1, -1, -1, 0);
    }
    ...
    private void ShowStats_Knockback(UIElement element)
    {
      if (!element.IsMouseHovering)
        return;
      Main.instance.MouseText(Language.GetTextValue("BestiaryInfo.Knockback"), 0, (byte) 0, -1, -1, -1, -1, 0);
    }
    ...
    private void ShowStats_Life(UIElement element)
    {
      if (!element.IsMouseHovering)
        return;
      Main.instance.MouseText(Language.GetTextValue("BestiaryInfo.Life"), 0, (byte) 0, -1, -1, -1, -1, 0);
    }
  }
}
