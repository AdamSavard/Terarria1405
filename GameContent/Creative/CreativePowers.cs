...
namespace Terraria.GameContent.Creative
{
  public class CreativePowers
  {
      ...
      private void Deserialize_SyncEveryone(BinaryReader reader, int userId)
      {
        ...
        else
        {
          for (int index1 = 0; index1 < count; ++index1)
          {
            ...
            {
              int playerIndex = index1 * 8 + index2;
              if (playerIndex != Main.myPlayer)
              {
                if (playerIndex < this._perPlayerIsEnabled.Length)
                  this.SetEnabledState(playerIndex, bitsByte[index2]);
                else
                  break;
              }
            }
      ...
      public void OnPlayerJoining(int playerIndex)
      {
        ...
        for (int index1 = 0; index1 < num; ++index1)
        {
          ...
          for (int index2 = 0; index2 < 8; ++index2)
          {
            ...
            if (index3 < this._perPlayerIsEnabled.Length)
              bitsByte[index2] = this._perPlayerIsEnabled[index3];
            else
              break;
          }
    ...
    public class ModifyTimeRate : CreativePowers.ASharedSliderPower, IPersistentPerWorldContent
    {
      ...
      {
        this.TargetTimeRate = (int) Math.Round((double) Utils.Remap(this._sliderCurrentValueCache, 0.0f, 1f, 1f, 24f, true));
      }
      ...
      public override UIElement ProvideSlider()
      {
        ...
        UIText uiText1 = new UIText("x24", 1f, false);
        uiText1.HAlign = 1f;
        uiText1.VAlign = 0.0f;
      ...
      private void UpdateSliderAndShowMultiplierMouseOver(UIElement affectedElement)
      {
        ...
        Main.instance.MouseTextNoOverride(originalText, 0, (byte) 0, -1, -1, -1, -1, 0);
      }
    }
    ...
    public class DifficultySliderPower : CreativePowers.ASharedSliderPower, IPersistentPerWorldContent
    {
      ...
      internal override void UpdateInfoFromSliderValueCache()
      {
        this.StrengthMultiplierToGiveNPCs = (double) this._sliderCurrentValueCache > 0.330000013113022 ? Utils.Remap(this._sliderCurrentValueCache, 0.33f, 1f, 1f, 3f, true) : Utils.Remap(this._sliderCurrentValueCache, 0.0f, 0.33f, 0.5f, 1f, true);
      ...
      {
        UIImage uiImage1 = new UIImage((Asset<Texture2D>) Main.Assets.Request<Texture2D>(iconImagePath, (AssetRequestMode) 1));
        uiImage1.HAlign = 1f;
    ...
    public class ModifyWindDirectionAndStrength : CreativePowers.ASharedSliderPower
    {
      ...
      public override UIElement ProvideSlider()
      {
        ...
        uiPanel.Append((UIElement) slider);
        uiPanel.OnUpdate += new UIElement.ElementEvent(CreativePowersHelper.UpdateUseMouseInterface);
        UIText uiText1 = new UIText(Language.GetText("CreativePowers.WindWest"), 1f, false);
        uiText1.HAlign = 1f;
        uiText1.VAlign = 0.0f;
        uiPanel.Append((UIElement) uiText1);
    ...
    public class ModifyRainPower : CreativePowers.ASharedSliderPower
    {
      ...
      public override UIElement ProvideSlider()
      {
        ...
        uiPanel.Append((UIElement) slider);
        uiPanel.OnUpdate += new UIElement.ElementEvent(CreativePowersHelper.UpdateUseMouseInterface);
        UIText uiText1 = new UIText(Language.GetText("CreativePowers.WeatherMonsoon"), 1f, false);
        uiText1.HAlign = 1f;
        uiText1.VAlign = 0.0f;
        uiPanel.Append((UIElement) uiText1);
    ...
    public class SpawnRateSliderPerPlayerPower : CreativePowers.APerPlayerSliderPower, IPersistentPerPlayerContent
    {
      ...
      {
        return this._cachePerPlayer.IndexInRange<float>(playerIndex) && (double) this._cachePerPlayer[playerIndex] == 0.0;
      }
      ...
      {
        return (double) sliderValue < 0.5 ? Utils.Remap(sliderValue, 0.0f, 0.5f, 0.1f, 1f, true) : Utils.Remap(sliderValue, 0.5f, 1f, 1f, 10f, true);
      }
      ...
      public override UIElement ProvideSlider()
      {
        ...
        uiPanel.Append((UIElement) slider);
        uiPanel.OnUpdate += new UIElement.ElementEvent(CreativePowersHelper.UpdateUseMouseInterface);
        UIText uiText1 = new UIText("x10", 1f, false);
        uiText1.HAlign = 1f;
        uiText1.VAlign = 0.0f;
        uiPanel.Append((UIElement) uiText1);
