...
namespace Terraria.GameContent.Creative
{
  public class ItemsSacrificedUnlocksTracker : IPersistentPerWorldContent, IOnPlayerJoining
  {
    ...
    public void SetSacrificeCountDirectly(string persistentId, int sacrificeCount)
    {
      ...
      int index;
      if (!ContentSamples.ItemNetIdsByPersistentIds.TryGetValue(persistentId, out index))
        return;
      this.SacrificesCountByItemIdCache[index] = num;
      this.MarkContentsDirty();
    }
    ...
    public void Load(BinaryReader reader, int gameVersionSaveWasMadeOn)
    {
      ...
      for (int index1 = 0; index1 < num1; ++index1)
      {
        ...
        this._sacrificeCountByItemPersistentId[key] = num2;
        int index2;
        if (ContentSamples.ItemNetIdsByPersistentIds.TryGetValue(key, out index2))
          this.SacrificesCountByItemIdCache[index2] = num2;
      }
    }
