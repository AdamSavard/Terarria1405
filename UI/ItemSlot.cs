...
namespace Terraria.UI
{
  public class ItemSlot
  {
    ...
    {
      return Main.npcShop > 0 && (type == 854 || type == 3035);
    }
...
label_59:
          if ((context == 23 || context == 24) && Main.netMode == 1)
            NetMessage.SendData(121, -1, -1, (NetworkText) null, Main.myPlayer, (float) player.tileEntityAnchor.interactEntityID, (float) slot, 0.0f, 0, 0, 0);
          if (context == 26 && Main.netMode == 1)
          {
            NetMessage.SendData(124, -1, -1, (NetworkText) null, Main.myPlayer, (float) player.tileEntityAnchor.interactEntityID, (float) slot, 0.0f, 0, 0, 0);
            break;
          }
          break;
        case 2:
...
label_95:
          if (context == 25 && Main.netMode == 1)
            NetMessage.SendData(121, -1, -1, (NetworkText) null, Main.myPlayer, (float) player.tileEntityAnchor.interactEntityID, (float) slot, 1f, 0, 0, 0);
          if (context == 27 && Main.netMode == 1)
          {
            NetMessage.SendData(124, -1, -1, (NetworkText) null, Main.myPlayer, (float) player.tileEntityAnchor.interactEntityID, (float) slot, 1f, 0, 0, 0);
            break;
          }
          break;
        case 3:
      ...
      Player player)
    {
      ...
      switch (context)
      {
        ...
        case 27:
          ...
          {
            bool flag2 = false;
            if (!flag2 && (Main.mouseItem.stack < Main.mouseItem.maxStack && Main.mouseItem.type > 0 || Main.mouseItem.IsAir) && (inv[slot].type > 0 && (Main.mouseItem.type == inv[slot].type || Main.mouseItem.IsAir)))
            {
              flag2 = true;
    ...
    public static void DrawRadialCircular(SpriteBatch sb, Vector2 position)
    {
      ...
      for (int index = 0; index < player.CircularRadial.RadialCount; ++index)
      {
        ...
        if (player.CircularRadial.SelectedBinding == index)
          num6 = 1.7f;
        sb.Draw(texture2D, position + vector2, new Rectangle?(), color * num6, 0.0f, texture2D.Size() / 2f, num2 * num6, SpriteEffects.None, 0.0f);
        if (binding != -1)
        {
