...
namespace Terraria
{
  public class Lang
  {
    ...
    {
      return id >= 0 && id < 145 && Lang._emojiNameCache[id] != null ? Lang._emojiNameCache[id] : LocalizedText.Empty;
    }
    ...
    public static string AnglerQuestCountChat(NPC npc)
    {
      ...
      return Language.SelectRandom(Lang.CreateDialogFilter("AnglerQuestChatter.", substitutionObject), (UnifiedRandom) null).FormatWith(substitutionObject);
    }
    ...
    {
      object substitutionObject = Lang.CreateDialogSubstitutionObject(npc);
      return Language.SelectRandom(Lang.CreateDialogFilter("GolferChatter.", substitutionObject), (UnifiedRandom) null).FormatWith(substitutionObject);
    }
    ...
    {
      object substitutionObject = Lang.CreateDialogSubstitutionObject(npc);
      return Language.SelectRandom(Lang.CreateDialogFilter("CatChatter.", substitutionObject), (UnifiedRandom) null).FormatWith(substitutionObject);
    }

    public static string DogChat(NPC npc)
    {
      object substitutionObject = Lang.CreateDialogSubstitutionObject(npc);
      return Language.SelectRandom(Lang.CreateDialogFilter("DogChatter.", substitutionObject), (UnifiedRandom) null).FormatWith(substitutionObject);
    }
      ...
      bool showAllAvailableUnits)
    {
      ...
      if (time.Hours > 0)
      {
        ...
        if (!showAllAvailableUnits && time > TimeSpan.FromHours(1.0))
          ++hours;
        string str2 = str1 + (object) hours + (abbreviated ? (object) (" " + Language.GetTextValue("Misc.ShortHours")) : (hours == 1 ? (object) " hour" : (object) " hours"));
        if (!showAllAvailableUnits)
          return str2;
        str1 = str2 + " ";
      }
      if (time.Minutes > 0)
      {
        ...
        if (!showAllAvailableUnits && time > TimeSpan.FromMinutes(1.0))
          ++minutes;
        string str2 = str1 + (object) minutes + (abbreviated ? (object) (" " + Language.GetTextValue("Misc.ShortMinutes")) : (minutes == 1 ? (object) " minute" : (object) " minutes"));
        if (!showAllAvailableUnits)
          return str2;
        str1 = str2 + " ";
      }
