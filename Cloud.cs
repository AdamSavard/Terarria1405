...
namespace Terraria
{
  public class Cloud
  {
    ...
    public void Update()
    {
      ...
      if ((double) this.position.X + (double) TextureAssets.Cloud[this.type].Width() * (double) this.scale < -(double) num5 || (double) this.position.X > (double) Main.screenWidth + (double) num5)
        this.active = false;
