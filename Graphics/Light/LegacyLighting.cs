...
namespace Terraria.Graphics.Light
{
  public class LegacyLighting : ILightingEngine
  {
    ...
    private void PreRenderPhase()
    {
      ...
      if (Main.wofNPCIndex >= 0)
      {
        if (Main.player[Main.myPlayer].gross)
        {
          try
          {
            ...
            {
              LegacyLighting.LightingState[] state = this._states[index1 - num11];
      ...
      for (int index1 = x; index1 < num19; ++index1)
      {
        LegacyLighting.LightingState[] state = this._states[index1 - this._expandedRectLeft];
        for (int index2 = y; index2 < num20; ++index2)
        {
          LegacyLighting.LightingState lightingState = state[index2 - this._expandedRectTop];
          ...
          Vector3 color;
          this._lightMap.GetLight(index1 - x, index2 - y, out color);
          if (this._rgb)
          {
          ...
          {
            float num8 = (float) (((double) color.X + (double) color.Y + (double) color.Z) / 3.0);
            lightingState.R2 = num8;
            lightingState.G2 = num8;
            lightingState.B2 = num8;
          }
          switch (this._lightMap.GetMask(index1 - x, index2 - y))
          {
          ...
          {
            int num8 = index1 - this._expandedRectLeft;
            int num9 = index2 - this._expandedRectTop;
            if (this._minX > num8)
              this._minX = num8;
            if (this._maxX < num8 + 1)
              this._maxX = num8 + 1;
            if (this._minY > num9)
              this._minY = num9;
            if (this._maxY < num9 + 1)
              this._maxY = num9 + 1;
          }
        }
      }
      ...
      {
        int index1 = (int) tempLight.Key.X - this._requestedRectLeft + Lighting.OffScreenTiles;
        int index2 = (int) tempLight.Key.Y - this._requestedRectTop + Lighting.OffScreenTiles;
        if (index1 >= 0 && (double) index1 < (double) this._camera.UnscaledSize.X / 16.0 + (double) (Lighting.OffScreenTiles * 2) + 10.0 && (index2 >= 0 && (double) index2 < (double) this._camera.UnscaledSize.Y / 16.0 + (double) (Lighting.OffScreenTiles * 2) + 10.0))
        {
          LegacyLighting.LightingState lightingState = this._states[index1][index2];
          ...
          if (this._minX > index1)
            this._minX = index1;
          if (this._maxX < index1 + 1)
            this._maxX = index1 + 1;
          if (this._minY > index2)
            this._minY = index2;
          if (this._maxY < index2 + 1)
            this._maxY = index2 + 1;
        }
      }
      if (!Main.gamePaused)
